...
def _parseAccountDict(response):
    ...
    tag = response.get('ssoImageTags',None)
    if status == 'SUCCESS' and tag:
        save_account(response['loginCredentials'])
    else:
        username = response['loginCredentials']['username']
        print('{}验证失败'.format(username))
        pass
